...
export const Form = () => {
    ...
    const getData = (page) => {
        ...
        fetch(`http://localhost:3002/user?_page=${page}&_limit=5`)
            .then((res) => {
    ...
    const sortLH = () => {
        ...
        fetch(`http://localhost:3002/user?_sort=salary&_order=asc&_page=${page}&_limit=5`)
            .then((res) => res.json())
            .then((res) => {
                // res.sort((a, b) => a.salary - b.salary) increment sort karne ke liye
                setForm(res);
            })
            .catch((err) => {
                console.log(err);
    ...
    const sortHL = () => {
        ...
        fetch(`http://localhost:3002/user?_sort=salary&_order=desc&_page=${page}&_limit=5`)
            .then((res) => res.json())
            .then((res) => {
                // res.sort((a, b) => b.salary - a.salary) decrement
                setForm(res);
            })
            .catch((err) => {
                console.log(err);
    ...
    const filterDepartment = (depart) => {
        ...
        fetch(`http://localhost:3002/user?department=${depart}&_page=${page}&_limit=5`)
            .then((res) => res.json())
            .then((res) => {
    ...
    return (
        <>
            ...
            <div className="pageBox">
                <p className="text-center h6 my-3">Page No: {page} </p>
